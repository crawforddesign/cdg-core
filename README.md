...
- Agency still bypasses every rule unconditionally, exactly as it bypassed the per-role hide checkboxes before. Agency recognition is broader now: the account is Agency if any of the *configured* Agency Email, the *hard-coded* default `support@crawforddesigngp.com`, or the username `cdgsupport` matches — case-insensitive, and independent of the **Enable Custom Roles** toggle for the visibility bypass. The auto-Administrator-assignment side still requires custom roles to be on, since that's a role-management concern.
